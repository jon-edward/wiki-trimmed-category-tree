...
__all__ = ("CategoryTree", "less_than_page_count_percentile")


def less_than_page_count_percentile(
    cat_tree: CategoryTree, page_percentile: int
) -> List[int]:
    cutoff = np.percentile(
        list(cat_tree.nodes[x]["page_count"] for x in cat_tree.nodes), page_percentile
    )
...
class CategoryTree(networkx.DiGraph):
    def __init__(
        self,
        assets_or_dataframes: Union[
            Assets, Tuple[pd.DataFrame, pd.DataFrame], None
        ] = None,
        *,
        delete_without_attributes: bool = True,
        **attr,
    ):
        super().__init__(**attr)
        ...
        if isinstance(assets_or_dataframes, Assets):
            ...
            raise TypeError(
                f"Invalid type for CategoryTree input: {type(assets_or_dataframes)}"
            )
    ...
    def _add_dataframes(self, dataframes: Tuple[pd.DataFrame, pd.DataFrame]):
        category_iter = zip(
            dataframes[0]["category_id"],
            dataframes[0]["name"],
            dataframes[0]["page_count"],
        )

        for category_id, name, page_count in category_iter:
            self.add_node(
                category_id, category_id=category_id, name=name, page_count=page_count
            )
        ...
        categories = pd.DataFrame(
            {
                ...
                "page_count": [id_to_page_count.get(c[0], -1) for c in ordered_items],
            }
        )

        edges = pd.DataFrame(
            {"from_id": [e[0] for e in _edges], "to_id": [e[1] for e in _edges]}
        )
